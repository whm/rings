    ...
    foreach my $thisPID (@pid_list) {
        ...
        while (my $row = $sth->fetchrow_hashref) {
            ...
            if ($width==0 || $height==0) {
                ...
                debug_output (" Skipping image for $thisPID");
                return;
            }

            if ($opt_debug) {
                debug_output("pid: $thisPID");
                debug_output("Camera: $camera");
                debug_output("DateTime: $this_datetime");
                debug_output("FStop: $this_fnumber");
                debug_output("Shutter Speed: $this_shutterspeed");
            }
