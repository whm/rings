<!-- ################# Navigation Sidebar header ################ -->

<div id="maintNav">
     <a href="/rings/index.php"><img
       src="/rings-images/icon-home.png"
       alt="Pick a New Ring"
       border="0"></a>
     <br/>
     Pictures
     <ul>
     <li><a href="picture_load.php">Load</a></li>
     <li><a href="picture_reload.php">Re-Load</a></li>
     <li><a href="picture_maint.php">Maint</a></li>
     <li><a href="picture_sort.php">Sort</a></li>
     <li><a href="picture_sort.php?in_new=1">Sort New</a></li>
     <li><a href="ring_slide_table.php">Sort Table</a></li>
     </ul>
     People
     <ul>
     <li><a href="people_search.php">Find</a></li>
     <li><a href="people_maint.php">Maint</a></li>
     </ul>
     Groups
     <ul>
     <li><a href="groups.php">List</a></li>
     <li><a href="group_maint.php">Maint</a></li>
     </ul>
</div>
<div id="maintContent">
<h2><?php print $thisTitle;?></h2>

<!-- ################# Navigation Sidebar header ################ -->
